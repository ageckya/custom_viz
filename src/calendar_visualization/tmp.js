...
const drawCalendar = (props) => {
  ...
  props.data.forEach((d) => {
    ...
    d3.select("#D" + format(d.date))
      ...
      .select("title")
      .text(format(d.date) + ": " + valueFormatted) // popup info with date and value
      .on("click", LookerCharts.Utils.openDrillMenu(d.value.links));
  });

// attempt to add in cell labels
  var bar = svg
    .selectAll(".label")
      .data(data) //(data) is the matrix loaded in via csv
    .enter()
    .append("g")
    .attr("class", "label")
  ...
  bar
    .append("text")
    .attr("x", (d) => x(d.group))  //group is the first column in the input csv
    .attr("y", (d) => y(d.variable)) //variable is the second column in the input csv
    .attr("dy", ".35em")
    .text((d) => d.value); // the text to display in each cell - value is a column in the input csv
});
// end of my edit
